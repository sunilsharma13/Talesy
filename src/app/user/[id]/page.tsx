...
export default function UserPage({ params }: Props) {
  ...
  useEffect(() => {
    async function fetchUserData() {
      ...
      try {
        ...
        const followerRes = await fetch(`/api/users/${params.id}/followers/count`);
        if (followerRes.ok) {
        ...
        const followingRes = await fetch(`/api/users/${params.id}/following/count`);
        if (followingRes.ok) {
  ...
  return (
    <main className="p-6">
      ...
      {/* Add user's posts here if needed */}
    </main>
  );
}